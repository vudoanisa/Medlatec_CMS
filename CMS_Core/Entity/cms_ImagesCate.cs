...
{
    [Serializable]
    public class cms_ImagesCate : IValidatableObject
    {
        #region Constructors  
        public cms_ImagesCate() { }
        #endregion
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            {
                results.Add(new ValidationResult("Mời bạn nhập vào chuyên mục ảnh"));
            }
            else if (this.vName.Length > 200)
            {
                results.Add(new ValidationResult("Chuyên mục ảnh lớn hơn 200 ký tự"));
            }

            if (string.IsNullOrEmpty(this.vTitle))
            {
                results.Add(new ValidationResult("Mời bạn nhập tiêu đề chuyên mục ảnh"));
            }
            else if (this.vTitle.Length > 200)
            {
                results.Add(new ValidationResult("Tiêu đề chuyên mục ảnh lớn hơn 200 ký tự"));
            }
            ...
            if (!string.IsNullOrEmpty(vDescription))
            {
                ...
                {
                    results.Add(new ValidationResult("Mô tả chuyên mục ảnh lớn hơn 500 ký tự"));
                }
            }
            ...
            if (!string.IsNullOrEmpty(vKeyword))
            {
                ...
                {
                    results.Add(new ValidationResult("Từ khóa chuyên mục ảnh lớn hơn 200 ký tự"));
                }
            }
